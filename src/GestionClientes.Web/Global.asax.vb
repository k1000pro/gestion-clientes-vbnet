...
Public Class Global_asax
    ...
    ''' <summary>
    ''' Registra las excepciones no controladas y, aparte, el contenido no permitido que rechaza la
    ''' validación de petición. Con customErrors en RemoteOnly, un cliente remoto solo ve la página
    ''' de error genérica; en la máquina local se conserva el detalle para diagnosticar. El
    ''' registro es la fuente fiable en ambos casos.
    '''
    ...
    '''
    ''' La URL se obtiene aparte, dentro de su propio Try, y antes de distinguir el caso de
    ''' contenido no permitido: este manejador puede dispararse en contextos donde Request no está
    ''' disponible (típicamente un fallo durante el arranque de la aplicación), y ahí Request.Url
    ''' lanza HttpException. Si eso ocurriera dentro de la interpolación del mensaje, el manejador
    ''' de errores fallaría y ocultaría la excepción original, que es justo lo que existe para
    ''' evitar.
    ''' </summary>
